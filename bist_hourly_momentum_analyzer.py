...
def get_bist_symbols():
    """
    Türkiye borsasındaki (BIST) hisse senetlerinin sembollerini döndürür.
    """
    # BIST'teki bazı büyük şirketlerin sembolleri
    ...
    symbols = [
        ...
        'ALARK.IS', 'DOHOL.IS', 'ENKAI.IS', 'OTKAR.IS', 'PETKM.IS',
        'SASA.IS', 'HEKTS.IS', 'OYAKC.IS', 'SOKM.IS' ,'TTKOM.IS',
        'ASTOR.IS', 'ULKER.IS', 'TKFEN.IS', 'ODAS.IS', 'ENJSA.IS',
        'AKSEN.IS', 'CIMSA.IS', 'GUBRF.IS'
    ]
    return symbols
...
def scan_bist_for_momentum(period='1d', lookback_days=180, verbose=False):
    """
    Tüm BIST hisselerini tarar ve momentum kriterlerini karşılayanları bulur
